...
public class CommentController {
    ...
    @Operation(summary = "Create Comment", description = "해당 게시물에 댓글 생성", responses = {
            @ApiResponse(responseCode = "201", description = "Create Success", content = @Content(schema = @Schema(implementation = CommentResponseDto.class)))})
    @PostMapping
    ...
    @Operation(summary = "Find all Comments", description = "해당 게시물 모든 댓글 가져오기", responses = {
            @ApiResponse(responseCode = "200", description = "Find Success", content = @Content(schema = @Schema(implementation = CommentResponseDto.class)))})
    @GetMapping
    ...
    @Operation(summary = "Update Comment", description = "댓글 수정", responses = {
            @ApiResponse(responseCode = "200", description = "Update Success", content = @Content(schema = @Schema(implementation = CommentResponseDto.class)))})
    @PutMapping
    @ResponseStatus(HttpStatus.OK)
    ...
    @Operation(summary = "Delete Comment", description = "해당 id 댓글 삭제", responses = {
            @ApiResponse(responseCode = "200", description = "Delete Success")})
    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public Long deleteComment(@Parameter(description = "댓글 id", in = ParameterIn.PATH) @PathVariable Long id) {
